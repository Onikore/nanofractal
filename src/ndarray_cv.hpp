...
// NOTE: nb::ndarray with constrained template args (e.g. const uint8_t,
// c_contig) SILENTLY COERCES mismatched inputs (casting dtype / making a
// contiguous copy) rather than raising. That would defeat both zero-copy and
// input validation. So we take an unconstrained RawArray as the parameter type
// and validate dtype + C-contiguity ourselves (raising TypeError), then
// zero-copy wrap in as_mat().
...
inline void validate_image_array(const RawArray &arr) {
  ...
  // vacuously contiguous, so skip the check in that case. A size-1 dimension
  // may also carry a stride of 0 (numpy uses this as an optimisation) — that
  // is still logically C-contiguous because no movement is required.
  if (arr.ndim() >= 1) {
    for (size_t i = 0; i < arr.ndim(); ++i)
      if (arr.shape(i) == 0)
        return; // empty array is contiguous
    size_t expected = 1;
    for (int i = (int)arr.ndim() - 1; i >= 0; --i) {
      int64_t s = arr.stride(i);
      // Allow stride==0 when the dimension has only one element.
      bool ok = (s == (int64_t)expected) || (arr.shape(i) == 1 && s == 0);
      if (!ok) {
...
inline cv::Mat as_mat(const RawArray &arr) {
  ...
  if (arr.ndim() == 3 && arr.shape(2) == 1) {
    // (H,W,1) has the same memory layout as (H,W) — zero-copy single-channel.
    return cv::Mat((int)arr.shape(0), (int)arr.shape(1), CV_8UC1, data);
  }
  if (arr.ndim() == 3 && arr.shape(2) == 3) {
    return cv::Mat((int)arr.shape(0), (int)arr.shape(1), CV_8UC3, data);
  }
  throw nb::value_error("image must be uint8 (H,W), (H,W,1) grayscale or "
                        "(H,W,3) BGR, C-contiguous");
}

// Like as_mat but also rejects degenerate (0-size) images. Detection entry
// points use this so an empty frame yields a clear ValueError instead of an
// opaque OpenCV assertion failure deep in the detector.
...
inline nb::ndarray<nb::numpy, int32_t>
ids_to_numpy(std::vector<int32_t> &&ids) {
  size_t n = ids.size();
