...
public:
    ...
    enum Dict:int{
        ARUCO_MIP_36h12=0, APRILTAG_36h11=1,
        DICT_4X4_50=2,  DICT_4X4_100=3,  DICT_4X4_250=4,  DICT_4X4_1000=5,
        DICT_5X5_50=6,  DICT_5X5_100=7,  DICT_5X5_250=8,  DICT_5X5_1000=9,
        DICT_6X6_50=10, DICT_6X6_100=11, DICT_6X6_250=12, DICT_6X6_1000=13,
        DICT_7X7_50=14, DICT_7X7_100=15, DICT_7X7_250=16, DICT_7X7_1000=17,
    };
    // Total grid size (inner bits + 2 border cells) for each dict family.
    static int dictGridSize(Dict d){
        if(d>=DICT_4X4_50 && d<=DICT_4X4_1000) return 6;
        if(d>=DICT_5X5_50 && d<=DICT_5X5_1000) return 7;
        if(d>=DICT_7X7_50 && d<=DICT_7X7_1000) return 9;
        return 8; // 6x6 inner: ARUCO_MIP_36h12, APRILTAG_36h11, DICT_6X6_*
    }
    ...
    static inline std::vector<Marker> detect(const cv::Mat &img,unsigned int maxAttemptsPerCandidate=10,Dict dict=ARUCO_MIP_36h12,DetectorParams params=DetectorParams{}){
        return _detect( img,maxAttemptsPerCandidate,dict,params,nullptr);
    }
    // Cached overload: pass codes precomputed with dictCodes(dict) to skip the
    // per-call table construction. `dict` is still used for the grid size.
    static inline std::vector<Marker> detect(const cv::Mat &img,unsigned int maxAttemptsPerCandidate,Dict dict,DetectorParams params,const std::vector<uint64_t>& codes){
        return _detect( img,maxAttemptsPerCandidate,dict,params,&codes);
    }
private:
    //obfuscate start
    static inline std::vector<Marker> _detect(const cv::Mat &img,unsigned int maxAttemptsPerCandidate,Dict dict,DetectorParams params,const std::vector<uint64_t>* codesCache);
...
std::vector<Marker>  MarkerDetector::_detect(const cv::Mat &img, unsigned int maxAttemptsPerCandidate, Dict dict, DetectorParams params, const std::vector<uint64_t>* codesCache){
    ...
    if(_scale<1.f) _minContour = std::max(4, int(_minContour*_scale + 0.5f));
    cv::adaptiveThreshold(procImage, thresImage, 255.,cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY_INV, _ablock, params.adaptive_c);
    ...
    cv::Mat bits(_gsize,_gsize,CV_8UC1);
    ...
    if(DetectedMarkers.size()>0){
        ...
        if(_scale<1.f)
            for(auto &m:DetectedMarkers)
                for(auto &pt:m){ pt.x*=_inv_scale; pt.y*=_inv_scale; }
...
int MarkerDetector:: getMarkerId(const cv::Mat &bits, int &nrotations, const std::vector<uint64_t> &dict){
    ...
    const int G = bits.cols;            // full grid side (6..9)

    //first check that outer is all black
    for(int x=0;x<G;x++){
        if( bits.at<uchar>(0,x)!=0)return -1;
        if( bits.at<uchar>(G-1,x)!=0)return -1;
        if( bits.at<uchar>(x,0)!=0)return -1;
        if( bits.at<uchar>(x,G-1)!=0)return -1;
    }

    // Inner code as a flat 0/1 array on the stack (max 7x7 = 49). Working purely
    // on this avoids the per-rotation cv::Mat heap allocations the original did.
    const int N = G - 2;                // inner grid side (4..7)
    uint8_t cur[49], tmp[49];
    for(int r=0;r<N;r++){
    ...
    // Pack inner array -> uint64 (bit 0 = bottom-right; scan bottom-to-top,
    // right-to-left within a row) -- matches the dictionary code encoding.
    auto pack=[N](const uint8_t* g)->uint64_t{
        uint64_t result = 0; int bidx = 0;
        for(int y=N-1;y>=0;y--)
            for(int x=N-1;x>=0;x--)
                result |= (uint64_t)g[y*N+x] << bidx++;
        return result;
    };
    // 90 deg rotation matching the original: out(i,j) = in(N-1-j, i).
    auto rot90=[N](const uint8_t* in, uint8_t* out){
        for(int i=0;i<N;i++)
            for(int j=0;j<N;j++)
                out[i*N+j] = in[(N-1-j)*N + i];
    };

    nrotations = 0;
    uint8_t* g = cur; uint8_t* o = tmp;
    for(;;){
        uint64_t id = pack(g);
        for(size_t i=0;i<dict.size();i++)
            if( dict[i]==id )
                return (int)i;//done!
        if(++nrotations >= 4) break;
        rot90(g,o);
        std::swap(g,o);
    }
    return -1;
